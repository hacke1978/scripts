...
tok{end} = [];
fullDir=fullfile(tok{:});
fullDir = [ '/' fullDir];
for ex=1:length(extList)
    filename = fullfile(fullDir, extList{ex});
    pathLFP = dir(fullfile(filename, '*stimOn.lfp'));
    pathMUAX = dir(fullfile(filename, '*stimOn.muax'));
    pathSpike = dir(fullfile(filename, sprintf('*%s_chopped.spike', allCfg.tag)));
...
clear muax lfp spike
muax.data = muaAll; lfp.data = lfpAll; spike = spikeAll;
...
%% Get all conditions
Cond = Cond(find(taccept));
trialsClean = find(taccept);
condLib = unique(Cond);

if strcmp(allCfg.type, 'grating-ori')
    CondSF = CondSF(find(taccept));
